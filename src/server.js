            ...
            foodRecord: serializer.extend({
                ...
                serialize(resource, request) {
                    ...
                    return json.map(record => ({
                        ...
                        calories_eaten: Math.round(parseInt(record.amount)*parseInt(record.food.calories)/100),
                        protein_eaten: Math.round(parseInt(record.amount)*parseInt(record.food.proteins)/100),
                        fat_eaten: Math.round(parseInt(record.amount)*parseInt(record.food.fats)/100),
                        carbs_eaten: Math.round(parseInt(record.amount)*parseInt(record.food.carbs)/100)
                    }))
                }
            }),
    ...
    // TODO: add brands and categories
        routes() {
            ...
            this.get("/users/:id/food_records/:date", (schema, request) => {
                return schema.foodRecords.where({userId: request.params.id, date: request.params.date})
            });
        },
    })
    return server;
